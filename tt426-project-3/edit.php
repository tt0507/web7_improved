...
if (isset($_POST["delete_image"])) {
    ...
    $sql = "DELETE FROM image_tags WHERE images_id = :delete_image_id";
    $params = array(":delete_image_id" => $delete_image_id);
    exec_sql_query($db, $sql, $params);
...
if (isset($_POST['submit_tag'])) {
    ...
    if ($tag_filter == NULL && $exist_tag == NULL) {
        ...
        if (isset($_POST["exist_tag"])) {
            // Get tag of submitted picture
            $sql = "SELECT tags.id FROM tags LEFT OUTER JOIN image_tags ON tags.id = image_tags.tag_id LEFT OUTER JOIN images ON image_tags.images_id = images.id WHERE images_id = :id";
            $params = array(":id" => $id_pic);
            $tag_elements = exec_sql_query($db, $sql, $params)->fetchAll(PDO::FETCH_ASSOC);

            $check_dups_tag = array();
            foreach ($tag_elements as $tag_element) {
                array_push($check_dups_tag, $tag_element['id']);
            }

            // $check_dups_tag contains tags(id) of sent picture

            if (!in_array($exist_tag, $check_dups_tag)) {
                ...
                add_message('Added Tag to Image');
            } else {
                add_message('Tag already attached for image');
            }
        }

        if (isset($_POST["a_tag"]) && $tag_filter != NULL) {
            ...
            if (!in_array($convert_tag_filter, $check_duplicate)) {
                ...
                add_message_2('Successfully added New Tag');
            } else {
                add_message_2('Newly Created Tag Already exists');
            }
        }
    }
}
            ...
            <div class="push_left">
                ...
                <form id="new_tag" action="edit.php" method="post">
                    ...
                    <?php if (isset($_POST['submit_tag'])) { ?>
                        ...
                        <p class='red'><?php show_message_2() ?></p>
                    <?php
